...
import logging
import threading
from pymysql.connections import Connection as conn
    ...
    def __exit__(self, exc, value, traceback):
        ...
        if self._pool:
            if not exc or exc in self._reusable_expection:
                ...
                try:
                    self.close()
                    logging.warning("Close not reusable connection from pool(%s) caused by %s", self._pool.name, value)
                except Exception:
                    pass

    def _recreate(self, *args, **kwargs):
        conn = Connection(*args, **kwargs)
        logging.debug('Create new connection due to pool(%s) lacking', self._pool.name)
        return conn
...
class ConnectionPool:
    ...
    def __init__(self, size=5, name=None, *args, **kwargs):
        ...
        self.name = name if name else '-'.join([kwargs.get('host', 'localhost'), str(kwargs.get('port', 3306)),
    ...
    def get_connection(self, timeout=1, retry_num=1) -> conn:
        """
        ...
        try:
            ...
            logging.debug('Get connection from pool(%s)', self.name)
            return conn
        except queue.Empty:
            if retry_num > 0:
                self._THREAD_LOCAL.retry_counter += 1
                logging.debug('Retry get connection from pool(%s), the %d times', self.name,
    ...
    def put_connection(self, conn):
        ...
        try:
            ...
            logging.warning("Put connection to pool(%s) error, pool is full, size:%d", self.name, self.size())
